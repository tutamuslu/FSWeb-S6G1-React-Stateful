...
export default function Programcilar() {
  ...
  const öneCikanProgramci = programciListesi.find((dev) => dev.id === programciId) 
  return öneCikanProgramci.isim
  };

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: programciId ? 'royalblue': 'gold'// 🤔 kutlarken renk gold'a dönecek
  };
